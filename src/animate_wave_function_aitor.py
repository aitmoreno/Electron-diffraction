...
from WaveFunction_CN_aitor import WaveFunction
...
def print_analytic_modulus():
    x, y, a, x0, y0, kx0, ky0, t = symbols('x y a x0 y0 kx0 ky0 t')
    sigma = sympy.sqrt(a**2 + t**2/(4*a**2))
    expr = 1/(sigma*sympy.sqrt(2*sympy.pi)) * sympy.exp(-1/2*((x-x0-(kx0)*t)/sigma)**2) * 1/(sigma*sympy.sqrt(2*sympy.pi)) * sympy.exp(-1/2*((y-y0-(ky0)*t)/sigma)**2)
    pprint(expr)
...
anim.save('2D_2slit_dx={0}_dt={1}_yf1={2}_k={3}.gif'.format(dx,dt,abs(yf1),kx0), fps=15)
